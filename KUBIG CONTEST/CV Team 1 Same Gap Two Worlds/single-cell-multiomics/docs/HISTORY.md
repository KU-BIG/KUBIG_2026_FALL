...
## 2026-08-13 (계속 8) — 후속 분석: gap 지표 분해, 그리고 이전 결론 하나 정정

사용자가 "다음 단계 계속 진행"을 요청해서 이전에 제안한 3가지(gap 분해 분석, exp A를 multiome에도 적용, exp C 재현성 확인) 중 가장 빠른 것부터 시작.

**한 일:** `src/experiments/followup_gap_decomposition.py` 작성+실행 (계획서에는 없던 항목이라 "follow-up"으로 명명). `gap_report()`의 5개 지표(delta_gap, alignment, uniformity_a/b, linear_separability)가 각각 top5_retrieval_acc와 얼마나 상관되는지, phase1 baseline + exp A + exp C 결과를 모두 풀링해서(44행) 분석.

**결과 요약**

- **풀링 전체(44행) 기준으로는 5개 지표 모두 top5_retrieval_acc와 강한 음의 상관** (pearson r = -0.72 ~ -0.86, 전부 p<1e-7). 즉 "gap이 크면 성능이 낮다"는 직관적 관계가 전체적으로는 꽤 강하게 성립한다.
- **실험별로 나눠 보면 그림이 다르다**:
  - 실험 A(quantity 축, n=27): 모든 지표가 강한 음의 상관 (delta_gap r=-0.86, p<1e-8) — 다만 이건 50개→2,000개 구간의 큰 변화가 상관계수를 지배하는 것이고, 2,000→13,953 구간에서 gap은 계속 (조금) 줄어드는데 retrieval은 오히려 떨어지는 국소적 비단조성은 선형 상관계수에는 잘 안 잡힌다.
  - 실험 C(n=11): 상관이 뚜렷하게 약해진다 (delta_gap pearson r=-0.495, p=0.121, 유의하지 않음; spearman r=-0.745, p=0.008로는 유의). 표본이 작아서(11개) 이 정도 약화가 "진짜 다른 관계"인지 "그냥 검정력 부족"인지는 이 데이터만으로 확정하기 어렵다.
  - Phase 1 baseline(n=6, cite vs multiome): delta_gap 상관이 오히려 **양수**(r=0.811, p=0.05, 경계선) — 그런데 이건 사실상 "두 그룹(=두 pair) 비교"라 진짜 연속적인 관계라기보단 "multiome은 GEX-ADT보다 gap도 크고 retrieval도 좋다"는 단일 사실을 상관계수로 표현한 것에 가까움 (n=6이라 신뢰 구간이 매우 넓다는 점도 감안).
  - 다중회귀(5개 지표 동시 투입, 표준화)에서는 uniformity_a와 uniformity_b가 서로 반대 부호로 유의하게 나오는데, 5개 지표가 서로 강하게 상관돼 있어서(quantity 축이 전체 분산의 대부분을 차지) 다중공선성 때문에 개별 계수 해석은 신뢰하기 어렵다고 판단 — 이 결과는 참고만 하고 과잉해석하지 않기로 함.

**정정할 것 (중요 — 이전 기록의 오류를 바로잡음):**
2026-08-13(계속 7)에서 실험 C 결과를 적으며 "retrieval 성능은 오히려 matchedN 쪽이 대체로 더 높다... **gap과 downstream 성능이 여기서도 같은 방향으로 안 움직인다는 걸 재확인**"이라고 썼는데, 이건 잘못된 서술이었다. matchedN이 gap도 더 낮고(0.053~0.063) retrieval도 더 높다(0.221~0.276)는 건 오히려 **gap↓·성능↑라는 직관적(=예상되는) 관계와 일치하는 것**이고, "어긋난다"고 볼 근거가 아니었다. 방금 한 상관분석에서도 실험 C 내부의 delta_gap-retrieval 관계는 (약하지만) 음의 방향으로 나온다 — 즉 서로 다른 방향으로 안 움직인다는 이전 결론은 철회한다.

**"gap과 성능이 어긋난다"는 관찰 중 실제로 남는 것은 다음 2가지뿐이다:**
1. **Harmony on/off 비교** (2026-08-13 계속 5) — 이건 진짜다: gap은 거의 그대로인데(cite 0.047→0.048) retrieval은 절반 이하로 떨어짐(0.062→0.028). 하나의 조작(배치보정)이 gap에는 영향이 없으면서 성능에는 큰 영향을 준 유일하게 깨끗한 반례.
2. **실험 A quantity 축의 고유전자수 구간(2,000→13,953)** — gap은 계속(조금) 줄어드는데 retrieval은 오히려 떨어짐. 전체 축으로 보면 상관은 강한 음수지만, 이 구간만 떼어보면 방향이 반대다.

나머지(cite vs multiome pair 비교, 실험 C 전반)는 재검토 결과 "어긋난다"기보다 **표본이 작아서 상관이 약하게 나오거나, 애초에 예상 방향과 일치**하는 것으로 정정한다. 세션 요약을 다시 쓴다면 "gap과 성능은 대체로 같이 움직이지만, 배치보정처럼 gap과 무관한 축을 조작하거나 정보량이 극단적으로 많아지는 특정 구간에서는 분리될 수 있다"가 더 정확한 문장이다.

**다음 단계:** 실험 A를 GEX-ATAC(multiome) pair에도 적용 → 실험 C 재현성(다중 seed) 확인.
...
## 2026-08-13 (계속 10) — 실험 C 재현성 확인 완료 — 반증 결과가 seed-fluke가 아님을 확인

**한 일:** `phase2_expC_reproducibility.py` 실행 완료 (전체 조건 × 3 seed, matchedN은 seed마다 독립적으로 새로 subsample, 총 33회 학습). 2026-08-13(계속 7)에서 단일 seed로 발견한 "single-lineage가 matchedN보다 gap이 크다"(계획서 가설과 반대)는 결과가 seed 하나의 우연이 아닌지 확인하는 게 목적.

**결과**

| lineage | single 평균±표준편차 | matchedN 평균±표준편차 | single>matchedN 비율(3×3=9 seed쌍 중) |
|---|---|---|---|
...
**해석: 반증 결과는 seed-fluke가 아니라 견고하게 재현된다.** 5개 계통 중 4개(T_CD4, T_CD8, Myeloid_Mono, NK_ILC)는 seed·subsample을 바꿔도 **9번 중 9번 전부** single-lineage 쪽 gap이 matchedN보다 크다 — 이 정도면 우연으로 보기 어렵다. B_cell만 67%(9번 중 6번)로 상대적으로 약한 편이지만, 그래도 다수는 같은 방향이다. **결론: "heterogeneity를 낮추면 gap이 준다"는 계획서 가설은 이 데이터/구현에서 일관되게, 재현 가능하게 반증된다.** 2026-08-13(계속 7)의 결론을 그대로 확정 결론으로 승격.

참고로 T_CD8에서 single 쪽 표준편차가 유독 크다(±0.0208, 다른 계통은 ±0.003~0.010) — 표본 크기(10,663, 상대적으로 작은 편)와 세부 세포타입 9종이 섞여 있다는 점이 학습 안정성에 영향을 줬을 수 있음, 추가 조사 후보로 남김.

**세션 2회차(후속 분석) 마무리 요약:**
1. Gap 지표 분해 → 이전에 "실험 C에서 gap과 성능이 어긋난다"고 적은 결론이 틀렸음을 발견, 정정함.
2. 실험 A quantity 축 → GEX-ATAC에서도 거의 동일하게 재현됨 (일반적 현상으로 격상).
3. 실험 C 반증 결과 → 다중 seed로 재현성 확인 완료, 확정 결론으로 격상.

이걸로 사용자가 요청한 "다음 단계" 3가지를 모두 완료했다.
